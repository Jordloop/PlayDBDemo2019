...
public class EmployeeController extends Controller
{
    ...
    {
        TypedQuery<Employee> query = db.em()
                .createQuery("SELECT e FROM Employee e WHERE employeeId = :employeeId", Employee.class);
        ...
        TypedQuery<State> statesQuery = db.em()
                .createQuery("SELECT s FROM State s WHERE stateId = :stateId", State.class);
        statesQuery.setParameter("stateId", employee.getStateId() );
        State state = statesQuery.getSingleResult();



        TypedQuery<Employee> reportQuery = db.em()
                .createQuery("SELECT e FROM Employee e WHERE reportsToEmployeeId = :employeeId", Employee.class);
        reportQuery.setParameter("employeeId", employeeId);
        List<Employee> reports = reportQuery.getResultList();
